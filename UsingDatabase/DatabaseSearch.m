...
//assumes whole query is just or
...
//assumes the whole query is just and no or
...
-(void)invertEqualtoNotEqual{
    NSMutableString *string = [[NSMutableString alloc]initWithString:self.query];
    [string replaceOccurrencesOfString:@"%22%24eq" withString:@"%22%24neq" options:NSCaseInsensitiveSearch range:NSMakeRange(0, string.length)];
    self.query = string;
}
...
-(void)getFromCollection:(NSString *)kField {
    ...
    NSURLSessionDataTask* dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (error == nil) {
            ...
            if(self.delegate) {
                [self.delegate viewUpdated:self.objects];
            }
            [dataTask cancel];
        }
        else {
    ...
    NSURLSessionDownloadTask* task = [session downloadTaskWithURL:url completionHandler:^(NSURL *fileLocation, NSURLResponse *response, NSError *error) { //2
        if (!error) {
            ...
            if (!image) {
                ...
                if(self.delegate){
                    [self.delegate viewUpdated:@[image]];
                }
            }
        }
        else {
